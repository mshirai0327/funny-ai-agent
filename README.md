...
# または Electron アプリのディレクトリで直接起動
cd claude-notify-app
npm install
npm start
```

リポジトリ直下の `npm start` は `claude-notify-app` の `npm start` を呼び出すショートカットです。
起動するとシステムトレイに常駐します。OSログイン時の自動起動も設定されます。
依存を更新したあとや `node_modules` が古そうなときは、同じディレクトリでもう一度 `npm install` を実行してください。
...
```bash
code --install-extension /path/to/claude-notify.vsix
```

または VS Code の `Extensions: Install from VSIX...` から選択。

注意: `.vsix` に含まれるのは VS Code 拡張本体だけです。ポップアップ表示には、先に `claude-notify-app` を起動しておく必要があります。
...
開発中に挙動だけ確認したい場合は、VS Code でこのリポジトリを開いて `F5` を押すと Extension Development Host を起動できます。
